...
const ChatBotDemo = () => {
  ...
  // When wallet is connected, use x402 payment flow (USDC)
  // When signed in with Echo, use standard Echo credits flow
  const x402Chat = useChatWithPayment({
    ...
    onError: (error: any) => {
      console.error('x402 chat error: ', error);
    },
  });

  const echoChat = useChat();

  const isWalletMode = isConnected && walletClient;
  const chat = isWalletMode ? x402Chat : echoChat;
