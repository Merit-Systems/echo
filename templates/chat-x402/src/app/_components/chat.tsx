...
import { createPaymentHeader } from '@/lib/402/createPaymentHeader';
...
const ChatBotDemo = () => {
  ...
  const isWalletUser = isConnected && !user;
  const walletReady = !isWalletUser || !!walletClient;
  ...
  const fetchWithX402 = useCallback(
    async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (!walletReady || !walletClient) {
        throw new Error('Wallet client is not ready for x402 payments');
      }

      const headers = new Headers(init?.headers);
      headers.set('use-x402', 'true');
      ...
      const paymentBody = await response.text();
    ...
    [isWalletUser, walletReady, walletClient]
  );
      ...
      <div className="flex h-full min-h-0 flex-col">
        ...
        <PromptInput onSubmit={handleSubmit} className="mt-4 flex-shrink-0">
          ...
          <PromptInputToolbar>
            ...
            <PromptInputSubmit
              disabled={!input || (isWalletUser && !walletReady)}
              status={status}
            />
          </PromptInputToolbar>
        </PromptInput>
