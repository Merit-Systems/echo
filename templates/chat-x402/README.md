...
```bash
npx echo-start@latest --template chat-x402
```
...
- ✅ **Dual Authentication** - Connect with Echo credentials or a Web3 wallet
...
- ✅ **x402 Payments** - Wallet-based payment flow with `use-x402` and `x-payment`
...
│   ├── connection-selector.tsx   # Connection switcher for header and guard
│   └── ui/
...
- `ECHO_APP_ID` and `NEXT_PUBLIC_ECHO_APP_ID`
- `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`
- `X402_PAY_TO_ADDRESS` and `X402_ASSET_ADDRESS`
...
- **Authentication**: Echo auth + wallet connection (RainbowKit/Wagmi)
- **Web3**: RainbowKit, Wagmi, Viem, x402, TanStack Query
