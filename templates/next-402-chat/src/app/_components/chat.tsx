...
import { usePaymentMode } from '@/components/wallet/payment-mode';
...
const ChatBotDemo = () => {
  ...
  const x402Chat = useChatWithPayment({
    walletClient: walletClient as unknown as Signer,
    regenerateOptions: {
      body: {
        model: model,
        paymentMethod: 'x402',
  ...
  const activeChat = paymentMode === 'x402' ? x402Chat : echoChat;
