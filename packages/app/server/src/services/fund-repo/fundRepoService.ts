...
export function safeFundRepoIfWorthwhile(): ResultAsync<void, Error> {
  const repoId = process.env.MERIT_REPO_ID;
  if (!repoId) {
    return ResultAsync.fromSafePromise(
      Promise.reject(new Error('Missing required environment variables'))
  ...
  return getSmartAccount().andThen(({ smartAccount }) => {
    return fromPromise(
      smartAccount.listTokenBalances({
        network: 'base',
      }),
      error =>
        new Error(
          `Failed to list token balances: ${error instanceof Error ? error.message : 'Unknown error'}`
        )
    ).andThen(balances => {
      ...
      logger.info(
        `Base USDC balance is ${usdcBalanceFormatted} USD, funding repo`
      );

      return safeFundRepo(Number(usdcBalanceFormatted));
    });
  });
}
