...
export class LmChatEcho implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				required: true,
				default: 'gpt-4o',
				description: 'The model to use for the chat completion (e.g., gpt-4o, claude-3-5-sonnet-20240620)',
				placeholder: 'gpt-4o',
			},
			{
				displayName: 'Options',
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		if (authenticationMethod === 'echoApi') {
			...
			const credentials = await this.getCredentials('echoApi') as IEchoCredentials;
			apiKey = credentials.apiKey;
			baseUrl = credentials.baseUrl || 'https://echo.router.merit.systems';
		} else {
			// Use OAuth2 authentication
			const credentials = await this.getCredentials('echoOAuth2Api') as IEchoOAuth2Credentials;
			...
			const token = credentials.oauthTokenData?.access_token || 
						  credentials.access_token || 
						  credentials.accessToken ||
						  credentials.token;
			
			if (!token) {
				const errorMsg = 'OAuth2 access token not found in credentials. ' +
					'Available properties: ' + Object.keys(credentials).join(', ') +
					(credentials.oauthTokenData ? 
						'. OAuth token data properties: ' + Object.keys(credentials.oauthTokenData).join(', ') : 
